...
async def _build_union_scheme_summary(farmer_unions: list[str]) -> str:
    ...
    seen_scheme_unions: set[str] = set()
    for union_name in farmer_unions:
        canonical_union = canonical_union_name(union_name)
        if canonical_union in SUPPORTED_SCHEME_CONTEXT_UNIONS and canonical_union not in seen_scheme_unions:
            seen_scheme_unions.add(canonical_union)
            scheme_unions.append(canonical_union)
