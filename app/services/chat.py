...
def _extract_query_from_search_header(text: str) -> str:
    text = text or ""
    match = re.search(r"> Search Results for `([^`]+)`", text)
    if not match:
        match = re.search(r"No results found for `([^`]+)`", text)
    return (match.group(1) if match else "").strip()


def _truncate_snippet_sentence_safe(text: str, max_chars: int) -> str:
    cleaned = _normalize_ws(text)
    if len(cleaned) <= max_chars:
        return cleaned
    window_start = max(int(max_chars * 0.65), max_chars - 140)
    candidates = [cleaned.rfind(tok, window_start, max_chars) for tok in (". ", "? ", "! ", "; ")]
    cut = max(candidates)
    if cut < 0:
        cut = cleaned.rfind(" ", window_start, max_chars)
    if cut < 0:
        cut = max_chars
    return cleaned[:cut].rstrip() + "..."


def _extract_snippets_from_search_return(content: str) -> tuple[list[str], bool, bool]:
    ...
    parse_failed = bool(re.search(r"Search Results for", text)) and not snippets
    return snippets, False, parse_failed
...
def _extract_shadow_search_evidence(new_messages: list) -> dict:
    ...
    for tool_call_id in ordered_ids:
        ...
        for content in returns_by_call_id.get(tool_call_id, []):
            ...
            snippets, no_results, parse_failed = _extract_snippets_from_search_return(content)
            if no_results:
                call_no_results = True
            if parse_failed:
                parser_empty_calls += 1
    ...
    return {
        ...
        "parser_empty_calls": parser_empty_calls,
...
async def stream_chat_messages(
    ...
    suggestions_hybrid_enabled = bool(getattr(settings, "suggestions_hybrid_enabled", False))
    suggestions_turn_id = ""
    ...
    with session_ctx, _root_ctx:
        ...
        try:
                    ...
                    if moderation_data.category == "valid_agricultural" and persona == "farmer":
                        ...
                        try:
                            ...
                            background_tasks.add_task(
                                ...
                                target_lang,
                                pipeline_profile,
                                request_turn_id=suggestions_turn_id,
                                queued_wall_ms=suggestions_queued_wall_ms,
                                queued_monotonic_s=suggestions_queued_monotonic_s,
                            )
                            logger.info(
                                "suggestions_task_queued session_id=%s target_lang=%s profile=%s queued_wall_ms=%s",
                                session_id,
                                target_lang,
                                pipeline_profile,
            ...
            if suggestions_hybrid_enabled and suggestions_turn_id:
                try:
                    ...
                    logger.info(
                        ...
                        shadow_evidence["parser_empty_calls"],
                    )
                except Exception as e:
            ...
            logger.info(
                "chat_stream_complete session_id=%s profile=%s stream_complete_wall_ms=%s stream_elapsed_ms=%s",
                session_id,
                pipeline_profile,
