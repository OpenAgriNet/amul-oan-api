...
async def create_suggestions(session_id: str, target_lang: str = 'mr', profile_name: str = "oss"):
    """
    Create and save suggestions for a session
    """
    logger.info(f"Getting suggestions for session {session_id}")

    # Chat passes profile_name="oss". Suggestions use that profile's primary
    # SUGGESTIONS tier (OSS/gemma first); managed remains available via the
    # fallback walker when FALLBACK_ENABLED. If the named profile is absent,
    # the resolver fail-safes to managed.
