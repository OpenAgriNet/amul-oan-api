...
1. **Pre-translation**: If `source_lang` is `gu`, `hi`, `bn` or `mr` (Hindi, Bengali and Marathi each behind `HINDI_CHAT_ENABLED` / `BENGALI_CHAT_ENABLED` / `MARATHI_CHAT_ENABLED`), the query is translated to English via Anthropic Haiku.
...
Gujarati, Hindi, Bengali and Marathi input is pre-translated to English before moderation and before the agrinet agent runs. This keeps the translation-pipeline path as English-in / English-out for the core agent flow.
