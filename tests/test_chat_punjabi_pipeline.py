"""Punjabi chat uses the translation pipeline like Hindi: a pa question is
pretranslated to English for moderation and the agent, and the English answer is
translated back to Punjabi. PUNJABI_CHAT_ENABLED=false bypasses both gates."""
import asyncio
...
PUNJABI_QUERY = "ਮੇਰੀ ਗਾਂ ਨੂੰ ਕਿੰਨਾ ਪਾਣੀ ਪਿਲਾਉਣਾ ਚਾਹੀਦਾ ਹੈ?"
ENGLISH_QUERY = "How much water should I give my cow?"
ENGLISH_ANSWER = "Give 40 to 60 liters of clean water daily."
PUNJABI_ANSWER = "ਆਪਣੀ ਗਾਂ ਨੂੰ ਹਰ ਰੋਜ਼ 40 ਤੋਂ 60 ਲੀਟਰ ਸਾਫ਼ ਪਾਣੀ ਪਿਲਾਓ।"
...
def test_punjabi_source_uses_pretranslation_then_punjabi_output(monkeypatch):
    calls = _install_fakes(monkeypatch)

    result = _drive(PUNJABI_QUERY, "pa")

    assert result == PUNJABI_ANSWER
    assert calls["pretranslation"] == [{"text": PUNJABI_QUERY, "source_lang": "pa"}]
    ...
    assert calls["translation"][0]["target_lang"] == "pa"


def test_punjabi_kill_switch_bypasses_translation_pipeline(monkeypatch):
    calls = _install_fakes(monkeypatch)
    monkeypatch.setattr(chat_service.settings, "punjabi_chat_enabled", False)

    result = _drive(PUNJABI_QUERY, "pa")

    assert calls["pretranslation"] == []
    assert calls["translation"] == []
    assert calls["agent"] and calls["agent"][0]["deps"].query == PUNJABI_QUERY
    assert calls["agent"][0]["deps"].lang_code == "en"
    assert result == ENGLISH_ANSWER


def test_punjabi_kill_switch_bypasses_punjabi_identity_short_circuit(monkeypatch):
    calls = _install_fakes(monkeypatch)
    monkeypatch.setattr(chat_service.settings, "punjabi_chat_enabled", False)

    result = _drive("ਤੁਸੀਂ ਕੌਣ ਹੋ?", "pa")

    assert result == ENGLISH_ANSWER
    assert calls["pretranslation"] == []
    assert calls["translation"] == []
    assert calls["agent"] and calls["agent"][0]["deps"].query == "ਤੁਸੀਂ ਕੌਣ ਹੋ?"
    assert calls["agent"][0]["deps"].lang_code == "en"


def test_punjabi_kill_switch_keeps_moderation_declines_in_english(monkeypatch):
    calls = _install_fakes(monkeypatch)
    monkeypatch.setattr(chat_service.settings, "punjabi_chat_enabled", False)
    ...
    result = _drive("ਪੰਜਾਬੀ ਵਿੱਚ ਇੱਕ ਗੈਰ-ਖੇਤੀ ਸਵਾਲ", "pa")
