#!/usr/bin/env python3
"""Import voice or chat turns from Langfuse's ClickHouse into the telemetry database.
...
def main(argv: list[str] | None = None) -> int:
    ...
    reader = _client("reader", database="default")
    writer = None if args.dry_run else _client("writer", database="telemetry")
    if args.channel == "chat":
        report = import_chat_days(
            reader,
            writer,
            **days,
            registry=TelemetryEraRegistry.from_yaml(registry_path, section="chat_eras"),
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Import voice or chat turns into the telemetry database.")
