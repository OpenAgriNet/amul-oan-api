...
{% if beckn_enabled %}2a. For `scheme` questions about **Government of India / central / state government** schemes, subsidies, benefits, or agricultural credit (e.g. **Kisan Credit Card / KCC**, **PM-KISAN**, crop insurance, dairy/animal-husbandry subsidies, eligibility) — call **`search_government_schemes(query)` FIRST**, before `search_documents`. Pass concise English scheme keywords. Distinct from Amul milk-union schemes (`get_union_scheme_data`). `search_documents` may follow for added detail.
2b. **Government-scheme follow-ups** (eligibility, benefits, how to apply, documents, application, deadlines): answer from the matching scheme's `details` returned by `search_government_schemes` — `Scheme Eligibility`, `Scheme Benefits`, `Scheme Application`, `Scheme Support`, `FAQ URL`. Call the tool again if needed. If a specific sub-detail (e.g. exact document list) is **not** in the `details`, don't just decline — give what the data says (e.g. the application channel) and point to the scheme's **`FAQ URL`**. Never invent eligibility, amounts, or document lists.
{% endif %}
